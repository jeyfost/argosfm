...
function changePrice(block, good, input, rate) {
	...
	$.ajax({
		...
		success: function(response) {
			if(response == "a") {
				document.getElementById(block).innerHTML = "<span class='basic'><b>Цена: </b>" + Math.floor((price * rate).toFixed(2)) + " руб. " + parseFloat((price * rate).toFixed(2) - Math.floor((price * rate).toFixed(2))).toFixed(2).substr(2, 2) + " коп.</span>";
