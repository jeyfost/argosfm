...
<head>
    ...
    <title>Аргос-ФМ | Добавление новости</title>
				...
				<div id='notificationRegistrationWindowOuter' style='display: block;'>
					<div id='notificationRegistrationWindow'>
						<form id='registrationNotificationForm'>
							<center><span class='headerStyleRed'>Первый этап регистрации завершён!</span></center>
							<br /><br />
							<span class='basic'>Поздравляем! Вы успешно прошли первый этап регистрации. Для продолжения перейдите по ссылке из письма, высланного по адресу вашей электронной почты. Если новых писем нет, проверьте папку \"Спам\".</span>
							<br /><br />
							<center><input type='button' onclick='closeNotification()' value='OK' id='cancelButton' style='float: none;' /></center>
						</form>
					</div>
    ...
    <div id='content'>
    	<div id='basketBlock'>
        	<?php
				...
				if(empty($_REQUEST['id']))
				{
					echo "<span class='headerStyle'>Добавление новости</span>";
				}
				else
				{
					echo "<span class='headerStyle'>Редактирование новости</span>";
				}
            ...
            <?php
				...
				{
					case 'success':
						echo "<span class='basicGreen'>Новость успешно добавлена!</span>";
						break;
					case 'empty':
						echo "<span class='basicRed'>Для добавления новости необходимо заполнить все поля.</span>";
						break;
					case 'failed':
						echo "<span class='basicRed'>При добавлении новости произошла ошибка. Повторите попытку.</span>";
						break;
					default: break;
				}
            ...
            <form method='post' <?php if(empty($_REQUEST['id'])){echo "action='../scripts/addNews.php'";}else{echo "action='../scripts/editNews.php?id=".$_REQUEST['id']."'";} ?> id='newsForm' enctype='multipart/form-data'>
                ...
                <?php
					...
					if(empty($_REQUEST['id']))
					{
						echo "<input type='submit' value='Добавить' id='addNewsSubmit' onclick='nicEditors.findEditor(\"newsText\").saveContent();' />";
					}
					else
					{
						echo "<input type='submit' value='Редактировать' id='addNewsSubmit' onclick='nicEditors.findEditor(\"newsText\").saveContent();' />";
					}
            ...
            <?php
				...
				if(empty($_REQUEST['id']))
				{
					...
					<a href='admin.php' class='noBorder'>
						<div id='backToAdminButton'>Вернуться в панель администрирования</div>
					</a>
					";
				}
				else
				{
					echo "
						...
						<a href='../news.php?id=".$_REQUEST['id']."' class='noBorder'>
							<div id='backToCurrentNews'>Назад</div>
						</a>
					";
				}
